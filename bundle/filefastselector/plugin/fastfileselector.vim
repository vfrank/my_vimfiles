...
if !exists("g:FFS_paths")
   let g:FFS_paths = []
endif

if !exists("g:FFS_shellslash")
   let s:FFS_shellslash = "&shellslash"
else
   let s:FFS_shellslash = "g:FFS_shellslash"
endif
...
for cur_path in vim.eval("g:FFS_paths"):
   fileList.extend( scan_dir(cur_path, vim.eval("g:FFS_ignore_list")) )

#fileList_tmp = scan_dir("D:/ProgramFiles/Vim/vimfiles", vim.eval("g:FFS_ignore_list"))
#fileList.extend(fileList_tmp)

vim.command('let s:base_path_length=%d' % len(path.encode("utf-8")))
vim.command("let s:file_list=[]")

# determine how we should handle backslashes
backslash_repl = '\\\\'
if (vim.eval(vim.eval("s:FFS_shellslash")) != '0'):
   backslash_repl = '/'

for i in fileList:
	vim.command('let s:file_list+=[["%s","%s"]]' % (i[0].replace('\\', backslash_repl), i[1].replace('\\', backslash_repl)))
EOF
	let s:filtered_file_list = s:file_list

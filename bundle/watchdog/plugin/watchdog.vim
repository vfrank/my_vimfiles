...
function! s:UpdateStatusLine(is_in_focus, is_insert_mode)
    ...
    " hardcoded buffet buffer ignore. TODO: make configurable maybe?
    " (i mean, make configurable what buffers should we skip)

    if bufname('%') =~ '^buflisttempbuffer'
       return
    endif
   ...
   " line_curr = line('.'), lines_num = line('$'), col_curr  = col('.'), cols_num  = col('$')-1
    ...
    let stl_len += strlen(&fenc) + 1 " Fileencoding + 1 slash
    ...
    "let ret .= is_format_full ? ' [%{&ff}/%{v:lang}/%{&fenc}]' : (is_format_slim ? '' : ' [%{&ff}]')
    let ret .= is_format_full ? ' [%{&ff}/%{&fenc}]' : (is_format_slim ? '' : ' [%{&ff}]')

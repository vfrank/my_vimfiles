...
"{{{ event registe
exe 'au BufEnter '. g:scalc_title . ' py his.load_cmds()'
exe 'au VimLeave,BufLeave '. g:scalc_title . ' py his.save_cmds()'

exe 'au BufEnter '. g:scalc_title . ' py his.load_session([calc.names, calc._env])'
...
function! swisscalc#InitDummy() "{{{
    " this function needs to be called from plugin/swisscalc.vim, in order to

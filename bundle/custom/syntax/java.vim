...
if version < 600
   silent! syn include <sfile>:p:h/javaid.vim
else
   silent! syn include syntax/javaid.vim
endif
...
if !exists("java_ignore_javadoc") && main_syntax != 'jsp'
  ...
  if version < 600
    syn include <sfile>:p:h/html.vim
  else
...
syn region  javaParenT	transparent matchgroup=javaParen  start="("  end=")" contains=@javaTop,javaParenT1,javaUserFunc
syn region  javaParenT1 transparent matchgroup=javaParen1 start="(" end=")" contains=@javaTop,javaParenT2,javaUserFunc contained
syn region  javaParenT2 transparent matchgroup=javaParen2 start="(" end=")" contains=@javaTop,javaParenT,javaUserFunc  contained
...
" by dfrank: types is starting from 1 or 2 capital letters, and continues with
" usual letters and digits. Or just one capital letter (like 'R')
syn match javaType   "\v<_{0,1}[A-Z]+[a-z][a-zA-Z0-9_]*"
syn match javaType   "\v<[A-Z]>"

" functions, methods
syn match javaUserFunc "\v(^\s*\#\s*define\s*)@<!<\h\w*>(\s|\n)*\("me=e-1 contains=javaType
...
if version >= 508 || !exists("did_java_syn_inits")
  ...
  JavaHiLink javaUserFunc               Function
  ...
  JavaHiLink javaDocTags		SpecialComment2
  JavaHiLink javaDocParam		SpecialComment

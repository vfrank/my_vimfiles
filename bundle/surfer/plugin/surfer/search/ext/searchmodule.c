 ...
 * incremented by 1. The vim variable `extension_version` in the file
        ...
        for (j = 0; j < PyList_Size(matchers); j++) {
            ...
            if (cond) {
                ...
                if (needle_idx == needle_len) {
                    ...
                    if (best_similarity == -99 || s < best_similarity) {
                        best_similarity = s;
...
_similarity(int haystack_len, PyObject *positions, int boundaries_count)
{
    ...
    float gravity = (positions_sum/positions_len) / haystack_len;
    float compactness = .0;
    if (n > 0)

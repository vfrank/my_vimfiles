...
" BY DFRANK
function! phpcomplete#ParseArgs(sArgs)
   let lArgs = split(a:sArgs, '\v\s*\,\s*')

   let sRet = ''
   for cur_arg in lArgs
      if !empty(sRet)
         let sRet .= ', '
      endif
      let sRet .= '`<'.cur_arg.'>`'
...
function! phpcomplete#CompletePHP(findstart, base)
	...
	if scontext =~ '\(=\s*new\|extends\)\s\+$'
		...
		let final_menu = []
		for i in res
			let final_menu += [{'word': i, 'kind': 'c', 'icase': 1}]
		endfor
	...
	elseif scontext =~ '\(->\|::\)$'
		...
		if scontext =~ '->$' || scontext =~ '::'
         ...
         "call confirm("classname=".classname)
			if classname != ''
				let classlocation = phpcomplete#GetClassLocation(classname)
            "call confirm("classlocation=".classlocation)
			else
				let classlocation = ''
			endif
            ...
            if scontext =~ '::$'
               let variables = filter(deepcopy(sccontent),
                        \ 'v:val =~ "^\\s*\\(\\(' . classAccess . '\\s\\+static\\|static\\)\\|var\\)\\s\\+\\$"')
            elseif scontext =~ '->$'
               let variables = filter(deepcopy(sccontent),
				...
				for i in start_list
					if has_key(c_variables, i)
						...
						let final_list +=
                        \ [{'word':substitute(i, '.*::', '', '').phpcomplete#ParseArgs(all_values[i]),
								\   'info':i.all_values[i].')',
								\   'kind':'f',
                        \   'icase': 1}]
					endif
            ...
            "call confirm ("file is not readable")
			endif
		...
		for i in start_list
            ...
            let final_list += [{'word':i, 'info':class.all_values[i], 'kind':'v', 'icase': 1}]
			else
				let final_list +=
                  \ [{'word':substitute(i, '.*::', '', '').phpcomplete#ParseArgs(all_values[i]),
						\   'info':i.all_values[i],
	...
	if a:base =~ '^\$'
		...
		for i in int_list
            ...
            let int_dict += [{'word':i, 'info':class.int_vars[i], 'kind':'v', 'icase': 1}]
			else
            let int_dict += [{'word':i, 'kind':'v', 'icase': 1}]
			endif
		endfor
		...
		for i in int_list
			if has_key(int_functions, i)
				let final_list +=
                  \ [{'word':i.phpcomplete#ParseArgs(int_functions[i]),
						\   'info':i.int_functions[i],
						\   'kind':'f',
                  \   'icase': 1}]
			elseif has_key(int_constants, i)
            let final_list += [{'word':i, 'kind':'d', 'icase': 1}]
			else
            let final_list += [{'word':i, 'icase': 1}]
			endif
		endfor
...
function! phpcomplete#GetClassLocation(classname) " {{{
	...
	while i < line('.')
		let line = getline(line('.')-i)
		if line =~ '\v\s*class\s+<' . a:classname  . '>'
         "call confirm("in-file")
			return expand('%:p')
		else
			let i += 1
			continue
	...
	for fname in tagfiles()
		...
		if classlocation != ''
            ...
            if !dfrank#util#IsAbsolutePath(classlocation)

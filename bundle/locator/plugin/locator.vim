...
let s:dItem_php_function = {
         \     'detect_type' : 'matchable',
         \     'detect_data' : {
         \        'start_regexp' : '\v<fu%[nction]>\!{0,1}\s+[^(]+\([^)]*\)',
         \        'get_end__eval' : 'call search("\\v\\{", "cW") | normal %',
         \     },
         \     'echo_type'   : 'func',
         \     'echo_data' : {
         \        'func_name' : 'Locator_EchoItem_PHPFunc',
         \     },
         \     'flags'   : 'C',
         \  }
...
function! Locator_EchoItem_PHPFunc(iLineNum)
   let sLine = getline(a:iLineNum)

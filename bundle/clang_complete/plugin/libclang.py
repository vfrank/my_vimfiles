...
def formatResult_subchunks(result, chunks, completion, cur_pos, is_optional):
  ...
  for chunk in chunks:
    ...
    # if this is subchunk of the optional chunk, then placeholders are for defaulted args.
    # Pity that clang does not return actual default values (currently)
    # so we just add " = ?" to the placeholder.
    if chunk.isKindPlaceHolder() and is_optional:
      chunk_spelling += " = ?"

    if chunk.isKindTypedText():
      completion['abbr'] = chunk_spelling

    

    chunk_len = len(chunk_spelling)
    if chunk.isKindPlaceHolder():
      completion['args_pos'] += [[ cur_pos, cur_pos + chunk_len ]]
    cur_pos += chunk_len
    completion['word'] += chunk_spelling


    #word += "SP{" + chunk.spelling + "}" + "S{" + str(chunk.string) + "}" + "--"
    #completion['word'] += "SP{" + chunk.spelling + "}" + "--"

    # if this is optional chunk, then it contains subchunks, and we need to handle them too.
    # Don't forget to give "True" for "is_optional" argument.
    if chunk.isKindOptional():
      cur_pos = formatResult_subchunks(result, chunk.string, completion, cur_pos, True)

  completion['menu'] = completion['word']

  # if we got returnValue, then prepend resulting string with it.
  if returnValue:
    completion['menu'] = returnValue.spelling + " " + completion['menu']
...
def formatResult(result):
  ...
  formatResult_subchunks(result, chunks, completion, 0, False)

  completion['info'] = completion['word']
  completion['dup'] = 0

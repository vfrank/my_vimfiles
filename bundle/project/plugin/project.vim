...
let s:return_to_window_cmd='wincmd p'
function! s:Project(filename) " <<<
    ...
    function! s:DoSetupAndSplit()
        ...
        silent! exec(s:return_to_window_cmd)
        if n == winnr()
            silent! wincmd l
        endif
        if n == winnr()
            ...
            exec(s:return_to_window_cmd) " Go back to the Project Window and ensure it is the right width
            exec b:proj_locate_command
            exec b:proj_resize_command
            exec(s:return_to_window_cmd)
        endif
    endfunction ">>>
    ...
    function! Project_GetAllFnames(recurse, lineno, separator)
        ...
        call Project_ForEach(a:recurse, a:lineno, "<SID>SpawnExec", a:separator, '')
        delfunction s:SpawnExec
    ...
    if !exists("g:proj_running")
        ...
        nnoremap <buffer> <silent> <C-S-Return> \|:call <SID>DoFoldOrOpenEntry('', 'vs')<CR>
        ...
        "nnoremap <script> <Plug>ProjectOnly :let lzsave=&lz<CR>:set lz<CR><C-W>o:Project<CR>:silent! exec(s:return_to_window_cmd)<CR>:let &lz=lzsave<CR>:unlet lzsave<CR>

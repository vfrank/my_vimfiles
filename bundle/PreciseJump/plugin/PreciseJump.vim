...
function! PreciseJumpF(lines_prev, lines_next, vismode)
    ...
    let re_rus = tr(re, s:dCharMaps['en'], s:dCharMaps['ru'])

    let re = escape(re, '.$^~')
    let re_rus = escape(re_rus, '.$^~')

    redraw
    call PreciseJump('\C\('.re.'\|'.re_rus.'\)', a:lines_prev, a:lines_next, a:vismode)
endfunction
...
function! s:FindTargets(re, line_numbers)
    ...
    for l in a:line_numbers
        ...
        " this correction is incremented when we replace multi-byte char with single-byte char (i.e. with key, like 'a', 'b', etc).
        let multibyte_correction = 0

        while 1
           let cur_line = getline(l)
           let match_start = match(cur_line, a:re, 0, n)
           let matched_str = matchstr(cur_line, a:re, 0, n)

           if match_start == -1
              break
           endif

           let tmp_str = strpart(cur_line, 0, match_start)
           let tmp_str_len = strchars(tmp_str)

           call add(targets, [l, tmp_str_len + 1, match_start + 1 - multibyte_correction])

           let multibyte_correction += (len(matched_str) - strchars(matched_str))
...
function! s:CreateHighlightRegex(coords)
    let tmp = []
    for [l, c, b] in s:Flatten(a:coords)
